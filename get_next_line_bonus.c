...
/*   Updated: 2024/12/20 16:33:41 by zbakour          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	read_save(int fd, char **str)
{
	...
	tmp_read = malloc((size_t)BUFFER_SIZE + 1);
	if (!tmp_read)
		return ;
	while (!ft_strchr(*str, '\n'))
	{
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > MAX_FD)
		return (NULL);
	read_save(fd, &buff[fd]);
	if (!buff[fd])
		return (NULL);
	next_line = get_string(buff[fd]);
	buff[fd] = handle_remaining(buff[fd]);
	return (next_line);
}
